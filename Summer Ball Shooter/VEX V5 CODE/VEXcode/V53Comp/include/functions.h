...
int ShootBall()
  {
	  while (1)
	  {
        ...
        if (Shoot==two)
        {
          ...
          {wait(10);}
          while(rpmError<60&&Shoot==two)// while the error<60 this means ball has been shot
          { 
          ...
          {
            if (BallTop.value(vex::percentUnits::pct)<68)
            {
              run(BallFeed,0);
              ballfeed=off;
              run(Roll,0);
...
{ int Counter=0;
	...
	{ 
		...
		{	
			if (Vision.objects[i].exists == 1&&fabs(GlobalFlagOffset - Vision.objects[i].centerX) < fabs(TXDist)) { TXDist = GlobalFlagOffset - Vision.objects[i].centerX;}
			else {}
		...
		if (TurnDiff > 0) { TurnDir = 1; }
           ...
           // float AngleHeight=Vision.objects[FinalObject].height;     
            if (SnapToFlag==1&&THeight>8)
             ...
             if(hood==on&&Counter%6==0)//if the flagsum has been added 50 times
             {Hood.startRotateTo((FlagSum/6)+20,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
             else if (hood==off){Hood.startRotateTo(20,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
...
{BallFeed.setTimeout(3,vex::timeUnits::sec);
 int count=0;   
    ...
    { 
        //////Front Roller
        if (Shoot==zero&&bL1){run(Roll,-10);}
        else if (Shoot==zero&&IR.value()==0&&BallBottom.value(vex::percentUnits::pct)<68&&BallTop.value(vex::percentUnits::pct)<68){run(Roll,0);}//1 1 1 
        else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)<68&&BallTop.value(vex::percentUnits::pct)<68){run(Roll,0);}//1 1 X
        else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)<68&&BallTop.value(vex::percentUnits::pct)>68&&IR.value()==0){run(Roll,70);}//0 1 1
        else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)>68&&BallTop.value(vex::percentUnits::pct)<68&&IR.value()==0){run(Roll,0);}//1 0 1
        else if (Shoot==zero&&IR.value()==0){run(Roll,70);} //X X 1
        else if (Shoot==zero&&roll==on){run(Roll,70);}
        else if (Shoot==zero){run(Roll,0);} //X X X



      //Ball LOader
      if (Shoot==0&&bY==1){run(BallFeed,-20);} 
      else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)<68&&BallTop.value(vex::percentUnits::pct)<67){run(BallFeed,0);}//1 1 X
      else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)<68&&IR.value()==0){run(BallFeed,30);}//0 1 1
      else if (Shoot==zero&&BallTop.value(vex::percentUnits::pct)<68&&IR.value()==0){run(BallFeed,0);}//1 0 1
      else if (Shoot==zero&&BallTop.value(vex::percentUnits::pct)<68){run(BallFeed,-80);}//1 0 0    
      else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)>68&&IR.value()==0){run(BallFeed,30);}//0 0 1
      else if (Shoot==zero&&BallBottom.value(vex::percentUnits::pct)<68){run(BallFeed,0);}//0 1 0
      else if (Shoot==zero){run(BallFeed,0);}

      int tempball=0;
      if (IR.value()==0) tempball++;
      if (BallBottom.value(vex::percentUnits::pct)<68) tempball++;
      if (BallTop.value(vex::percentUnits::pct)<68) tempball++;
      balls=tempball;
