...
int PrintScreen()
  {
      ...
      Brain.Screen.printAt(340, 140, "Dif %d ",Color);
...
int ShootBall()
  {
	  ...
	  while (1)
	  {
        ...
        if (Shoot==two)
        {
          ...
          {wait(10);}
          while(rpmError<65&&Shoot==two)// while the error<60 shoot ball
          { 
          ...
          while(rpmError>64&&Shoot!=zero) //wait for error to be above 80
          {
             run(BallFeed,0);
              ballfeed=off;
              run(Roll,0);
...
//Controller.Screen.print("%f",TurnDiff);
					double speed;
                    speed=pow(fabs(TurnDiff),1.1)*TurnDir*.3;
           ...
           // float AngleHeight=Vision.objects[FinalObject].height;     
            if (SnapToFlag==on&&THeight>8)
             ...
             {HIGH+=-0.3487*pow(THeight,2)+13.108*THeight+12.719; HIGHCount++;}
              else{MID+=-0.0329*pow(THeight,2)+1.6794*THeight+129.28;MIDCount++;}
               Counter++;
             ...
             if (hood==on&&Counter%15==0)
             {
               if (HIGHCount>MIDCount){FlagSum=HIGH/HIGHCount;}
               else{FlagSum=MID/MIDCount;}
               HIGH=0;MID=0;
               HIGHCount=0; 
               MIDCount=0;
             } 
              if(hood==on&&Counter%15==0&&(FlagSum)<165&&(FlagSum)>100)//if the flagsum has been added 50 times
             {Hood.startRotateTo((FlagSum),vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
             else if (hood==off){Hood.startRotateTo(120,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
             else if (hood==twoshot){Hood.startRotateTo(150,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
             if (Counter%15==0){FlagSum=0;}
                      wait(17);
            }
            if (hood==off||hood==on){Hood.startRotateTo(120,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
            else if (hood==twoshot){Hood.startRotateTo(150,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
...
int Driver()
{
 ...
 int count=0;
    ...
    { 
      ...
      if (Shoot==0)
      {
        if (bL1||roll==reverse){run(Roll,-85);}
...
{	    LM.resetRotation();
  ...
  int direction = 0;
	...
	T3 = 0;
	while (T3 < timeout)
	{ if (fabs(enc(RM))-5>fabs(enc(LM))){RO=.8;}
    else if(fabs(enc(LM))-5>fabs(enc(RM))){RO=1.2;}
    else {RO=1;}
		...
		if ((fabs(pidError) < 6) && (fabs(avgSpeed) < 10)) {
			StopDrive(brake);
			break;
		}
...
int Move(float speed, float dist,bool rampspeed,vex::brakeType X) {
	...
	double counter=100;
if(rampspeed){counter=20;}
	while (fabs(enc(LM)) < abs((dist * 360.0 / (4.0*3.14159)))) {


		float Roffset = 1.0;
		if (fabs(enc(LM)) < fabs((enc(RM)) + 5)) { Roffset = 0.9; }
		else if (fabs(enc(LM)) > fabs(enc(RM)) - 5) { Roffset = 1.1; }
		else {}
...
leftDrive(vel);
wait(100);
}while ((TLF!=enc(LM)||TRF!=enc(RM))&&(RLine.value(vex::percentUnits::pct)<50||LLine.value(vex::percentUnits::pct)<50)&&T3<1500);
StopDrive(coast);
SetDriveTorque(100);
}
////////////////////////////////////////////////////////////////////////////
