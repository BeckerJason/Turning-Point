...
task ShootBall()
{
	while(1)
	{
		if (Shoot == 1) //when button is pressed
		{
			...
			while (rpmError > 2) // while the dif between RPM and rpmGoal is less than 1 it can shoot
			{
			...
			while (rpmError <= 2 && SensorValue[BallTop] == 1) //rpm not in range yet and no ball waiting to be shot, roll up to see ball
			{
				motor[Chain] = 120;
				wait1Msec(10);
			}
