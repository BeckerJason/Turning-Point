...
int PrintScreen()
  {
      ...
      if (balls == 1) {Controller.Screen.print("1");}
      else if (balls == 2) {Controller.Screen.print("2");}
      else {Controller.Screen.print("0");}
...
//Controller.Screen.print("%f",TurnDiff);
           ...
           // float AngleHeight=Vision.objects[FinalObject].height;     
            ...
            {
              if (TYDist-PLATFORM<(-7.3*THeight+300.0))//high
              {HIGH+=-0.3487*pow(THeight,2)+13.108*THeight+12.719; HIGHCount++;}
              else
              {MID+=-0.0329*pow(THeight,2)+1.6794*THeight+129.28;MIDCount++;}
              Counter++;  
            } 
...
int Driver()
{
 ...
 int count=0;
    ...
    { 
      ...
      if (Shoot==0)
      {
        if (bL1||roll==reverse){run(Roll,-90);}
...
void TurnTurret(double degree)
  ...
  void TurnTurretTo(double degree)
  {   
    double gearratio=12.0/175.0;
    Turret.startRotateTo(degree/gearratio,vex::rotationUnits::deg,75,vex::velocityUnits::pct);
