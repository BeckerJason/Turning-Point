...
int PrintScreen()
  {
      ...
      Brain.Screen.printAt(340, 140, "Dif %f ",TurnDiff);
   ...
   if (Color==Blue){Vision.takeSnapshot(REDFLAG, TOTALSNAPSHOTS);}
    ...
    int TObject=9;
		...
		for (int i = 0; Vision.objects[i].exists == 1; i++)
		{
			...
			if ((Vision.objects[i].width > 8 && Vision.objects[i].height > 8) && (Vision.objects[i].centerX > (GlobalFlagOffset - 10 - TXDist) && Vision.objects[i].centerX < (GlobalFlagOffset + 10 - TXDist)) && Vision.objects[i].centerY < TYDist) 
      {
            TYDist = Vision.objects[i].centerY; TWidth = GlobalFlagOffset - Vision.objects[i].centerX; 
            THeight=Vision.objects[i].height;  
            TObject=i;
      }
      else{}
      } 
           ...
           // float AngleHeight=Vision.objects[FinalObject].height;     
            if (SnapToFlag==on&&THeight>8)
             ...
             {HIGH+=-0.3487*pow(THeight,2)+13.108*THeight+12.719; HIGHCount++;}
              else{MID+=-0.0329*pow(THeight,2)+1.6794*THeight+129.28;}
               Counter++;
             ...
             if (hood==on&&Counter%11==0)
             {
               if (HIGHCount>5){FlagSum=HIGH/HIGHCount;}
               else{;FlagSum=MID/(11-HIGHCount);}
               HIGH=0;MID=0;
               HIGHCount=0; 
             } 
              if(hood==on&&Counter%11==0&&(FlagSum)<180&&(FlagSum)>100)//if the flagsum has been added 50 times
             ...
             if (Counter%11==0){/*Counter=0;*/FlagSum=0;}
                      wait(17);
            }
...
int Driver()
{
 ...
 int count=0;
    ...
    { 
      ...
      if (Shoot==0)
      {
        if (bL1||roll==reverse){run(Roll,-100);}
...
if(rampspeed){counter=20;}
  ...
  if(brakebot){StopDrive(brake);}
  else{StopDrive(coast);}
  return 1;
}
...
leftDrive(vel);
wait(100);
}while ((TLF!=enc(LF)||TRF!=enc(RF))&&(RLine.value(vex::percentUnits::pct)<50||LLine.value(vex::percentUnits::pct)<50)&&T3<1500);
StopDrive(coast);
SetDriveTorque(100);
}
////////////////////////////////////////////////////////////////////////////
