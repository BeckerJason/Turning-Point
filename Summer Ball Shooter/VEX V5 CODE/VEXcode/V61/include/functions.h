...
int PrintScreen()
  {
      ...
      if (balls == one) {Controller.Screen.print("1");}
      else if (balls == two) {Controller.Screen.print("2");}
...
int ShootBall()
  {
	  ...
	  while (1)
	  {
        ...
        if (Shoot==two) 
        {
          ...
          while(rpmError<85&&Shoot==two)// while the error<60 shoot ball
          { MIDCount=0;
          ...
          while(rpmError>54&&Shoot!=zero) //wait for error to be above 80
          {HIGHCount=0;
             run(BallFeed,0);
              ballfeed=off;
              run(Roll,0);
         ...
         for (int i=0;rpmError<60&&Shoot!=zero;i++)
          {  //if (i==0) {wait(1000);}
            HIGHCount=0;
            run(BallFeed,100);
            run(Roll,100);
...
//Controller.Screen.print("%f",TurnDiff);
           ...
           // float AngleHeight=Vision.objects[FinalObject].height;     
            ...
            if(hood==on&&(FlagSum)>100&&(FlagSum)<200&&(HIGHCount==0&&MIDCount==0))//if the flagsum has been added 50 times
            {
              ...
              else if (hood==off){Hood.startRotateTo(124,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
              else if (hood==twoshot){Hood.startRotateTo(155,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
            else if (hood==manual){Hood.startRotateTo(ManualHood,vex::rotationUnits::deg,25,vex::velocityUnits::pct);}
            FlagSum=0;
              wait(25);
            }
            if (hood==off||hood==on){Hood.startRotateTo(124,vex::rotationUnits::deg,50,vex::velocityUnits::pct);}
